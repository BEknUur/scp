...
import { chatApi, linksApi, suppliersApi } from '@/api';
import { MessageOut, LinkOut, SupplierOut } from '@/types';
...
export default function ChatScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const [allSuppliers, allLinks] = await Promise.all([
        suppliersApi.listAll(),
        linksApi.listMyLinks(),
      ]);

      const supplierData = allSuppliers.find((s) => s.id === supplierId);
      const linkData = allLinks.find((l) => l.supplier_id === supplierId);

      if (!linkData) {
        Alert.alert('Error', 'No connection found with this supplier');
        router.back();
        return;
      }
          ...
          {messages.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptySubtext}>
                Start a conversation with {supplier?.name || 'this supplier'}
              </Text>
            </View>
          ) : (
            <FlatList
